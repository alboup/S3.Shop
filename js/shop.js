...
// Exercise 2
function cleanCart() {
  console.log('estic a cleanCart, i cartList= ')
  cart = []
  totalCarro = 0
  //console.log(cartList);
  printCart()
  ...
  for (var i = 0; i < cart.length; i++) {
    ...
    if (product.name === 'cooking oil' && quantity >= 3) {
      ...
      if (product.type === 'grocery' && DescompteGrocery == true) {
        ...
        //product.subtotalWithDiscount.toFixed(2) = quantity * price;
...
function printCart() {
  ...
  tableBody.innerHTML = ''
  total_price.innerHTML = totalCarro
  ...
  for (const product of cart) {
    ...
    productTotalCell.textContent = product.subtotalWithDiscount
